...
PRE_FFMPEG_VERSION=`wget https://repo.jellyfin.org/releases/server/debian/ffmpeg/ -q -O- | grep -o -P "([a-z0-9\-\.]+)(?=-bullseye_amd64.buildinfo)" | head -n 1`
...
CURRENT_VERSION=`cat package.json | grep -o -P "(?<=\"preversion\"\: \").*([\d\.])+"`
CURRENT_SHA=`cat package.json | grep -o -P "(?<=\"presha\"\: \").*([\d\.])+"`
